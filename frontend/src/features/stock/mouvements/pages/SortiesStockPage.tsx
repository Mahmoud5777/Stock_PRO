...
const PERMISSION_CODE = "SORTIES_STOCK";
...
export function SortiesStockPage() {
  const { page, setPage, search, setSearch, idSite, setIdSite, listQuery, createMutation } = useMouvementsStock("sorties");
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      await downloadSpreadsheet("/mouvements-stock/sorties/export", { search, filters: { idSite } }, "stock-out.xlsx");
  ...
  return (
    <div className="flex flex-col gap-5">
      <CrudPageHeader
        title="Stock Out"
        description="History of stock outflows, sales, and consumption."
        breadcrumb={[{ label: "Stock" }, { label: "Stock Out" }]}
        actions={
      ...
      <MouvementFormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} isSubmitting={createMutation.isPending} type="sorties" />
    </div>
  );
}
