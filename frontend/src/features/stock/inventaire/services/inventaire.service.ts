...
export const inventaireService = {
  async list(params: PageRequest & { idSite?: string }): Promise<Page<Inventaire>> {
    const { filters, idSite, ...rest } = params;
    const { data } = await apiClient.get<Page<Inventaire>>("/inventaires", { params: { ...rest, idSite, ...filters } });
    return data;
  },
